...
定义每个 Agent 由总控注入的内容、来源、读取方式、软预算。**读取方式**：`全文` = 完整注入；`摘要优先` = 先读 `## 摘要` 头，不足再 `read_file` 正文；`命中片段` = 按关键词从 `_index.md` 命中后定向取片段；`按类型切片` = 只取与本 Agent 职责相关的部分。
...
> **先读 `## 摘要`，摘要信息足够即可；需要逐条细节时再 `read_file` 读对应正文章节。**
...
3. 用当前**模块名 + 平台类型 + 测试类型关键词**匹配 `_index.md` 的归档标签：命中的归档条目 → 定向 `read_file` 取对应库该片段，按 §1 软预算注入给对应 Agent
...
- 注入的经验片段超过软预算时：只保留与本模块标签最相关的 **Top-K** 条，其余以一行提示替代：
  > 另有 N 条历史经验未注入，需要时 `read_file [库文件]`
